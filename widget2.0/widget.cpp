...
void Widget::initGame()
{
    ...
    // 随机生成3条不重复的竖琴线
    for (int i = 0; i < 3; i++) {
        int x = randGen.bounded(5, GRID_COLS - 8);
        int y = randGen.bounded(3, GRID_ROWS - 8);
        
        // 创建竖琴线对象，初始进度为0，未显示
        harpLines.append({x, y, 0, false});
    }

    int tx, ty;
    bool ok = false;
    while (!ok) {
        tx = randGen.bounded(5, GRID_COLS - 8);
        ty = randGen.bounded(3, GRID_ROWS - 8);

        ok = true;
        for (auto &hl : harpLines) {
            if (hl.x == tx && hl.y == ty) {
                ok = false;
                break;
            }
...
void Widget::checkLines()
{
    ...
    for (auto &line : harpLines) {
        // 计算竖琴线的坐标
        float lineX = line.x * cellW;
        float lineY = line.y * cellH;
        
        // 计算鼠标到竖琴线的距离
        float dist = qSqrt(qPow(mousePos.x() - lineX, 2) + qPow(mousePos.y() - lineY, 2));
        
        // 如果鼠标在竖琴线附近，并且竖琴线未显示
        if (dist < cellW * 3 && !line.revealed) {
            // 增加竖琴线的进度
            line.progress++;
    ...
    // 检查陷阱线触发
    float trapX = trapLine.x * cellW;
    float trapY = trapLine.y * cellH;
    float trapDist = qSqrt(qPow(mousePos.x() - trapX, 2) + qPow(mousePos.y() - trapY, 2));
...
void Widget::updateAudio()
{
    ...
    // 记录之前的触碰状态（现在暂时不使用，但保留用于将来扩展）
    bool wasTouching = isTouching;
    (void)wasTouching; // 消除未使用变量警告
...
void Widget::drawRules(QPainter &p)
{
    ...
    // 绘制规则文本，居中显示
    QRect infoRect(150, 250, 600, 300);
...
void Widget::drawGame(QPainter &p)
{
    ...
    // 绘制陷阱线（现在是圆形标记）
    float trapX = trapLine.x * cellW;
    float trapY = trapLine.y * (height() / (float)GRID_ROWS);
    int trapAlpha = trapLine.count * 10;
    p.setPen(QPen(QColor(255, 100, 100, trapAlpha), 3));
    p.setBrush(QColor(255, 100, 100, trapAlpha / 2));
    p.drawEllipse(QPointF(trapX, trapY), 18, 18);
